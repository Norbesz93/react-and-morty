...
function App() {
   ...
   const [locationPage, setLocationPage] = React.useState(2)
   ...
   const allLocations = async (locationPage) => {
      try {
         const currentloc = await axios.get("https://rickandmortyapi.com/api/location/?page=" + locationPage)
            for (const loc of currentloc.data.results) {
               scrollLocations.push(loc)
            }
            ...
            isShownCharacters ?
               <>        
                  <div className='charPage'>
                     <div className='char-wrap-div'>
                        {characters.results.map(character => <Characters character={character} /> ) }
                     </div>
                     <button className="btns" onClick={()=>setIsShownCharacters(false)}>Back</button>
                     { characterPage !== 1 && <i className="fas fa-angle-left prev" onClick={() => setCharacterPage(characterPage -1)}></i> }
                     { characterPage !== 42 && <i className="fas fa-angle-right next" onClick={() => setCharacterPage(characterPage +1)}></i> }
                  </div>
               </>
               :
               ...
               isShownLocations ?
                  <div className="locPage">
                     <div className={cube}>
                        <img className="side top flippety" id="gif" src={gif} alt="rickandmorty" />
                        <img className="side left flop" id="gif" src={gif} alt="rickandmorty" />
                     ...
                     >
                        {scrollLocations.map(location => <Locations cube={cube} setCube={setCube} location={location} />)}
                     </InfiniteScroll>
                     <button className="btns" onClick={() => setIsShownLocations(false)}>Back</button>
